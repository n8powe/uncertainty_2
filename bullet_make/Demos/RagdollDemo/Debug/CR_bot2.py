...
def run_training_CR(population, num_timesteps, training_list):
    ...
    sub.call("rm "+resultsFolder+"*",shell=True)  #This clears the directory
    ...
    if not done:
        ...
        for individual in population:
            C = 1 / ( sum( individual.cat ) / 7.0 )
            R = sum( individual.R ) / 7.0
            individual.fit = C * R
...
def run_testing_CR(test_list, population, timestep):
    ...
    newpath = r'Images/CR/Individuals_'+datetime.datetime.strftime(datetime.datetime.now(), '%Y-%m-%d')
    if not os.path.exists(newpath):
        os.makedirs(newpath)

    newpath2 = r'Images/CR/Individuals_'+datetime.datetime.strftime(datetime.datetime.now(), '%Y-%m-%d')+'/Individuals'+str(individual.uid)+'_data_for_neurons'
    if not os.path.exists(newpath2):
        os.makedirs(newpath2)
    ...
    for position in test_list:
        ...
        #save('Images/CR/Individuals_'+datetime.datetime.strftime(datetime.datetime.now(), '%Y-%m-%d')+'/Individuals'+str(individual.uid)+'_data_for_neurons/'+str(index)+'CnM_image_'+str(individual.uid)+'_'+datetime.datetime.strftime(datetime.datetime.now(), '%Y-%m-%d'))
        index = index + 1
...
def secondary_CR(newpopulation, POP_SIZE, num_timesteps, training_list):
    newpopulation = Fill_CR(newpopulation, POP_SIZE)
...
if __name__ == "__main__":
    population_dict = {}
    for i in range(0,1):
        newpop = iterations_CR(25, POP_SIZE, num_timesteps)
