...
def run_testing_CnM(test_list, population, timestep):
    ...
    newpath = r'Images/CnM/Individuals_'+datetime.datetime.strftime(datetime.datetime.now(), '%Y-%m-%d')
    if not os.path.exists(newpath):
        os.makedirs(newpath)

    newpath = r'Images/CnM/Individuals_'+datetime.datetime.strftime(datetime.datetime.now(), '%Y-%m-%d')+'/Individuals'+str(individual.uid)+'_data_for_neurons'
    if not os.path.exists(newpath):
        os.makedirs(newpath)
...
def secondary_CnM(newpopulation, POP_SIZE, num_timesteps, training_list):
    new_individuals = Fill_CnM(newpopulation, POP_SIZE)
    newpopulation = run_training_CnM(newpopulation, num_timesteps, training_list)
...
if __name__ == "__main__":
    population_dict = {}
    for i in range(0,1):
        newpop = iterations_CnM(3, POP_SIZE, num_timesteps)
