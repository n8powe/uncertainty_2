...
def run_training_CR(population, num_timesteps, training_list):
    ...
    newpath1 = (r'/Users/np/Desktop/Bullet/bullet_make/Demos/RagdollDemo/Debug/results')
    if not os.path.exists(newpath1):
        os.makedirs(newpath1)
    newpath2 = (r'/Users/np/Desktop/Bullet/bullet_make/Demos/RagdollDemo/Debug/positions')
    if not os.path.exists(newpath2):
        os.makedirs(newpath2)
    newpath3 = (r'/Users/np/Desktop/Bullet/bullet_make/Demos/RagdollDemo/Debug/weights')
    if not os.path.exists(newpath3):
        os.makedirs(newpath3)
    
    resultsFolder = "/Users/np/Desktop/Bullet/bullet_make/Demos/RagdollDemo/Debug/results/" # you may want to clear this directory before starting, e.g. by calling sub.call("rm "+resultsFolder+"*",shell=True)
    ...
    for individual in population:
        ...
        for training in training_list:
            ...
            gmlist.append(identifier)
            
            filename = identifier+"_weights_Matrix.csv"
            Send_Values_ToFile(parent, "/Users/np/Desktop/Bullet/bullet_make/Demos/RagdollDemo/Debug/weights/"+filename)
            Send_Values_ToFile_two(training, "/Users/np/Desktop/Bullet/bullet_make/Demos/RagdollDemo/Debug/positions/"+identifier+"_position.csv")
    ...
    while not done:
        ...
        if lsOutput: # if there are some files in resultsFolder...
            ...
            if ("gmvector" in firstFileName) or ("gerrorvector" in firstFileName):
                ...
                if (thisIdentifier in gmlist) or (thisIdentifier in gerrorlist):
                    ...
                    if "gmvector.txt" == thisFile:
                        gvarvectlist = []
                        movelist = []
                        for line in result:
                            move = float(line.split(",")[0])
                            uncert = float(line.split(",")[1])
                            gvarvectlist.append(move)
                            movelist.append(uncert)
                        individual.cert.append(gvarvectlist)
                        individual.move.append(movelist)
                        individual.R.append( np.corrcoef(np.asarray(movelist), np.asarray(gvarvectlist))[0][1] )
                        
                        gmlist.remove(thisIdentifier)
                        
                    if "gerrorvector.txt" == thisFile:
                        gmeanvectlist = []
                        for line in result:
                            gmeanvectlist.append(float(line))
                        individual.cat = gmeanvectlist
    ...
    if not done:
        ...
        for individual in population:
            individual.fit = sum(individual.cat)/7.0 * sum( individual.R )/7.0
...
def run_training_second_CR(population, POP_SIZE, num_timesteps, training_list):
    ...
    for individual in new_individuals:
        ...
        for position in training_list:
            ...
            fitness_R = np.corrcoef(mvect_list[:num_timesteps], gvect_list[:num_timesteps])[0][1]
...
def secondary_CR(newpopulation, POP_SIZE, num_timesteps, training_list):
    newpopulation = run_training_CR(newpopulation, num_timesteps, training_list)
...
if __name__ == "__main__":
    population_dict = {}
    for i in range(0,1):
        newpop = iterations_CR(10, POP_SIZE, num_timesteps)
