...
def run_training_CM(population, num_timesteps, training_list):
    ...
    while not done:
        ...
        if lsOutput: # if there are some files in resultsFolder...
            ...
            if ("mvector" in firstFileName) or ("gerrorvector" in firstFileName):
                ...
                if (thisIdentifier in mlist) or (thisIdentifier in gerrorlist):
                    ...
                    if "mvector.txt" == thisFile:
                        movelist = []
                        for line in result:
                            #for value in line:
                            movelist.append(float(line[0]))
                        individual.move.append(sum(movelist)/float(num_timesteps))
                        mlist.remove(thisIdentifier)
    ...
    if not done:
        ...
        for individual in population:
            C = 1.0 / ( sum( individual.cat ) / 7.0 )
            M = sum( individual.move ) / 7.0
            individual.fit = C * M
...
def run_testing_CM(test_list, population, timestep):
    #print os.getcwd()
    ...
    newpath = r'Images/CM/Individuals_'+datetime.datetime.strftime(datetime.datetime.now(), '%Y-%m-%d')
    if not os.path.exists(newpath):
        os.makedirs(newpath)
        
    newpath = r'Images/CM/Individuals_'+datetime.datetime.strftime(datetime.datetime.now(), '%Y-%m-%d')+'/Individuals'+str(individual.uid)+'_data_for_neurons'
    if not os.path.exists(newpath):
        os.makedirs(newpath)
    
    
    identifier = individual.uid
    #parent = parent.getValues()
    filename = str(identifier)+"_weights_Matrix.csv"
    Send_Values_ToFile(parent, filename)
    index = 0
    for position in test_list:
        ...
        #print position_size
        Send_Values_ToFile_two(position_size, "positions/"+str(identifier)+"_position.csv")
        

        #sub.Popen("./AppRagdollDemo %s"%identifier,shell=True)
        os.system("./AppRagdollDemo %s"%identifier)
        
        cat = open(path+"/results/"+str(identifier)+"_gerrorvector.txt", 'r')
        categorize = []
        for line in cat:
            categorize.append( float(line) )
        ...
        mvect = open(path+'/results/'+str(identifier)+'_mvector.txt', 'r')
        mvect_list = []
        for line in mvect:
            mvect_list.append(float(line))
        ...
        Send_Values_ToFile(values_save, 'Images/CM/Individuals_'+datetime.datetime.strftime(datetime.datetime.now(), '%Y-%m-%d')+'/Individuals'+str(individual.uid)+'_data_for_neurons/'+str(index)+str(individual.uid)+position[6]+position[7]+'Categorizeandmovement.txt')
        ...
        #save('/Users/np/Desktop/Bullet/bullet_make/Demos/RagdollDemo/Debug/Images/CM/Individuals_'+datetime.datetime.strftime(datetime.datetime.now(), '%Y-%m-%d')+'/Individuals'+str(individual.uid)+'_data_for_neurons/'+str(index)+'CnM_image_'+str(individual.uid)+'_'+datetime.datetime.strftime(datetime.datetime.now(), '%Y-%m-%d'))
        index = index + 1
    
    Send_Values_ToFile(parent, 'Images/CM/Individuals_'+datetime.datetime.strftime(datetime.datetime.now(), '%Y-%m-%d')+'/Individuals'+str(individual.uid)+'_data_for_neurons/'+str(index)+str(individual.uid)+position[6]+position[7]+'_ANN.txt')
...
def AGE_individuals_CM(population):
    for individual in population:
        individual = individual.age_()
    return population
        
def Inject_CM(population):
    population.append(Individual())
    return population        
    
def Fill_CM(population, POP_SIZE):
    new_individuals = []
    ...
    new_individuals = Inject_CM(new_individuals)
    return new_individuals

def del_doms_CM(population):
...
def Determine2_Dom_CM(population):
...
def dominate_CM(population):
    print "fit", [population[i].fit for i in range(0, len(population))][:10] 
    population = Determine2_Dom_CM(population)
    print "dom", [population[i].bool for i in range(0, len(population))]
    
    population = sort_by_dom_CM(population)
    population = sort_by_fit_CM(population)
    #print len(population)
    population = del_doms_CM(population)
    #print len(population)
...
def initial_CM(POP_SIZE, num_timesteps, training_list):
    oldpopulation = [Individual() for i in range(0, POP_SIZE)]
    oldpopulation = run_training_CM(oldpopulation, num_timesteps, training_list)
    newpopulation = dominate_CM(oldpopulation)
    return newpopulation
    
def secondary_CM(newpopulation, POP_SIZE, num_timesteps, training_list):
    new_individuals = Fill_CM(newpopulation, POP_SIZE)
    newpopulation = run_training_CM(newpopulation, num_timesteps, training_list)
    newpopulation = dominate_CM(newpopulation)
    return newpopulation
    
def iterations_CM(generations, POP_SIZE, num_timesteps):
    Send_Values_ToFile_two([num_timesteps], 'timesteps.txt')
    training_list, test_list = positions()
    for i in range(0, generations):
        print "Gen =", i
        if i == 0:
            newpopulation = initial_CM(POP_SIZE, num_timesteps, training_list)
            newpopulation = AGE_individuals_CM(newpopulation)
            newpopulation2 = newpopulation  
        if i > 0:
            newpopulation = secondary_CM(newpopulation, POP_SIZE, num_timesteps, training_list)
            newpopulation = AGE_individuals_CM(newpopulation)  
    
    run_testing_CM(test_list, newpopulation, num_timesteps) 
    #plt.show()
...
if __name__ == "__main__":
    population_dict = {}
    for i in range(0,1):
        newpop = iterations_CM(3, POP_SIZE, num_timesteps)
