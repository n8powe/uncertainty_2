...
// Return substring based on delimiter
string getSplit(string str, string delimiter) {
    
    string subString = str.substr(0, str.find(delimiter));
...
void RagdollDemo::initPhysics()
{
    ...
    // id is environmentID_individualID. Need to explode the id string and get the second part
    //std::string weights_file;
    
    //weights_file = ("/Users/np/Desktop/Bullet/bullet_make/Demos/RagdollDemo/Debug/"+identity+"weights_Matrix.csv").c_str();
    
    //std::cout << "UNIQUE ID IS " << id <<std::endl;   weights/"+id+"_
    ...
    const char *modes = "r";
    pos = fopen(("positions/"+id+"_position.csv").c_str(), modes);
...
void RagdollDemo::clientMoveAndDisplay()
{
    ...
    wts = fopen(("weights/"+id+"_weights_Matrix.csv").c_str(), mode);
    ...
    FILE *tim;
    const char *modess = "r";
    tim = fopen("timesteps.txt", modess);
        ...
        if (!pause || (pause && oneStep)) {
            ...
            for(int l=0; l < TICKS_PER_DISPLAY; l++) {     // COMMENT THIS OUT TO TURN GRAPHICS BACK ON
                ...
                if (timeStep==timesteps_import){
                    ...
                    gvar_and_mvect(g_var_vector, m_vector);
                    //printf("Movement: %f", movement);
                    exit(0);
...
void RagdollDemo::g_vector_save(double vector[1000]) {
    ...
    //std::cout << "***** UNIQUE ID IS *****" << id <<std::endl;
    
    ofstream outFile("results/"+id+"_gvarvector.txt");  //uncertainty
    if (outFile.is_open())
    {
    ...
    {
        cout << "Unable to save uncertainty file";
    }
}

void RagdollDemo::gvar_and_mvect(double vector1[1000], double vector2[1000])
{
    ofstream outFile("results/"+id+"_gmvector.txt");
    if(outFile.is_open()){
        for(int i=0; i<1000; i++){
            //for(int j=0; j<1000; j++){
            //outFile << new_vector[i][j] << ",";
            outFile<<vector1[i]<<","<<vector2[i] <<std::endl;//<<newline;
            //cout<<new_vector[0][0]<<","<<new_vector[0][1]<<newline;
        }
        outFile.close();
    }
    else
    {
        cout << "Unable to save gm vector file";
    }
}
...
void RagdollDemo::m_vector_save(double vector[1000]) {
    ...
    ofstream outFile("results/"+id+"_mvector.txt");
    if (outFile.is_open())
    {
        for (int count = 0; count < size; count ++){
            outFile << vector[count] << "\n";
    ...
    {
        cout << "Unable to save movement vector file";
    }
}
...
void RagdollDemo::g_mean_vector_save(double vector[1000]) {
    ...
    ofstream outFile("results/"+id+"_gerrorvector.txt");
    if (outFile.is_open())
    {
        for (int count = 0; count < size; count ++){
            outFile << vector[count] << "\n";
    ...
    {
        cout << "Unable to save g_mean_vector file";
    }
}
...
void RagdollDemo::guess_average_save(double vector[1000]) {
    ...
    ofstream outFile("results/"+id+"_gaveragevector.txt");
    if (outFile.is_open())
    {
        for (int count = 0; count < size; count ++){
            outFile << vector[count] << "\n";
    ...
    {
        cout << "Unable to save g_average file";
    }
}
...
void RagdollDemo::Save_Categorize(double categorize) {
    ...
    FILE *cat;
    char outputFilename[] = "categorize.csv";
...
void RagdollDemo::Save_Predict(double predict) {
    ...
    FILE *pre;
    char outputFilename[] = "predict.csv";
...
void RagdollDemo::Save_movement(double predict[1000]) {
    ...
    FILE *mov;
    char outputFilename[] = "movement.csv";
